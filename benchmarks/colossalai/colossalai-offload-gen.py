...
def get_colossalai_model(
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir=cache_dir, torch_dtype=dtype)
    hf_model.load_state_dict(state_dict)
    hf_model = hf_model.eval()

    accelerate.dispatch_model(hf_model, device_map=device_map, offload_dir=offload_dir)
    hf_model.model.embed_tokens.to(torch.cuda.current_device())
    hf_model.model.norm.to(torch.cuda.current_device())
    hf_model.lm_head.to(torch.cuda.current_device())
    return hf_model
