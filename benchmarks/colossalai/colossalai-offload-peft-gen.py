...
def get_colossalai_model(
    ...
    if cpu_offload:
        device_map = {
            ...
            "base_model.model.model.layers": "cpu",
            "base_model.model.model.norm.weight": 0,
            "base_model.model.lm_head.weight": 0,
        }
    if disk_offload:
        device_map = {
            "base_model.model.model.embed_tokens.weight": 0,
            "base_model.model.model.layers": "disk",
            "base_model.model.model.norm.weight": 0,
            "base_model.model.lm_head.weight": 0,
        }
    ...
    # NOTE(yongjunhe): ColossalAI moves the parameters with no gradient to CUDA
    model = GeminiDDP(model, offload_optim_frac=1.0, offload_param_frac=1.0)
    state_dict = model.state_dict()
    for name, param in state_dict.items():
        state_dict[name] = torch.nn.parameter.Parameter(param.data, param.requires_grad)
    del model
    gc.collect()

    hf_base_model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir=cache_dir, torch_dtype=dtype)
    hf_model = get_peft_model(hf_base_model, lora_config)
    hf_model.load_state_dict(state_dict)
    hf_model = hf_model.eval()

    accelerate.dispatch_model(hf_model, device_map=device_map, offload_dir=offload_dir)
    hf_model.base_model.model.model.embed_tokens.to(torch.cuda.current_device())
    hf_model.base_model.model.model.norm.to(torch.cuda.current_device())
    hf_model.base_model.model.lm_head.to(torch.cuda.current_device())
    return hf_model
